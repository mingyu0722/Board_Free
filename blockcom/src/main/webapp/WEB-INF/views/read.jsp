...
<body>
		...
		<tbody>
			...
			<tr>
				...
				<c:if test="${bf_contents != '등록되지 않은 글입니다.'}">
					<td colspan="2"
						style="word-break: break-all; word-wrap: break-word;"><c:set
							var="file" value="${file}" /> <c:if test="${file != null}">
							<c:forEach var="row" items="${file}">
								<c:if
									test="${row.file_ext == 'jpg' || row.file_ext == 'png' || row.file_ext == 'gif'}">
									<img src="${row.file_path}"
										style="width: 200px; height: 200px;" value="${row.file_name}" />
								</c:if>
							</c:forEach>
						</c:if>
						<p id="bf_contents">${read.bf_contents}</p></td>
				</c:if>
				<c:if test="${bf_contents == '등록되지 않은 글입니다.'}">
					<script>
						alert("등록되지 않은 글입니다.");
			...
			<c:if test="${file != null}">
				<c:forEach var="row" items="${file}">
					<tr>
						<td>첨부파일</td>
						<td><a href="${row.file_path}" download>${row.file_name}</a>&nbsp<font
							size="1em">(${row.file_size}KB)</font></td>
					</tr>
				</c:forEach>
			</c:if>
		</tbody>
	...
	<c:if test="${rec_flag}">
		<input type="checkbox" id="recommend" checked="checked"
			onchange="delRecommend()">추천</></c:if>
	<c:if test="${rec_flag eq false}">
		<input type="checkbox" id="recommend" onchange="recommend()">추천</>
	</c:if>
	<c:set var="recommendCnt" value="${read.recommendCnt}" />
	<c:if test="${recommendCnt != 0}">
		<font size="1em">(추천수 : ${read.recommendCnt})</font>
	</c:if>
	<br></br>
	<table border="1" style="width: 600px" name="preNextArticle">
		<tr>
			...
			<c:if test="${nextArticle != null}">
				...
				<c:if test="${use_sec == 'Y' }">
					<td><a href="#" onclick="nextArticle(); return false;">(비밀글)${nextArticle.bf_title}</td>
				</c:if>
				<c:if test="${use_sec == 'N' }">
					<td><a href="#" onclick="nextArticle(); return false;">${nextArticle.bf_title}</td>
				</c:if>
		...
		<tr>
			...
			<c:if test="${preArticle != null}">
				...
				<c:if test="${use_sec == 'Y'}">
					<td><a href="#" onclick="preArticle(); return false;">(비밀글)${preArticle.bf_title}</a></td>
				</c:if>
				<c:if test="${use_sec == 'N'}">
					<td><a href="#" onclick="preArticle(); return false;">${preArticle.bf_title}</td>
				</c:if>
	...
	<br>
	<h3>댓글</h3>
	<textarea id="bfr_contents" rows="4" cols="80"
		placeholder="댓글 입력(최대 300자)"></textarea>
	<br>
	...
	<c:forEach var="replyList" items="${replyList}">
		<table border="1" id="replyTable">
			<tr>
				<td rowspan="3"><textarea readonly
						class="bfr_contents_${replyList.bfr_idx}" rows="5" cols="60">${replyList.bfr_contents}</textarea>
					<button type="button" class="save_btn_${replyList.bfr_idx}"
						style="display: none;">확인</button></td>
				<td colspan="2">작성자: ${replyList.mem_name}</td>
			</tr>
			<tr>
				<td colspan="2">작성일: <c:set var="date"
						value="${replyList.mod_date}" /> <c:if test="${date == null}">
						<fmt:formatDate value="${replyList.reg_date}" pattern="yyyy-MM-dd" />
					</c:if> <c:if test="${date != null}">
						<fmt:formatDate value="${replyList.mod_date}" pattern="yyyy-MM-dd" />
					</c:if>
				</td>
			</tr>
			<tr>
				...
				<c:choose>
					<c:when test="${memIdx eq mem_idx || memIdx eq 1}">
						<td><button class="modify_btn" type="button"
								value="${replyList.bfr_idx}">수정</button></td>
						<td><button class="delete_btn" type="button"
								value="${replyList.bfr_idx}">삭제</button></td>
					</c:when>
					<c:otherwise></c:otherwise>
				</c:choose>
			</tr>
			<input type="hidden" class="mem_idx_${replyList.bfr_idx}"
				value="${replyList.mem_idx}" />
		</table>
	</c:forEach>
	<ul class="pagingUl">
		<c:if test="${prev}">
		...
		<c:forEach begin="${startPage}" end="${endPage}" var="idx">
			<li class='<c:out value="${idx == rpage? 'current':''}" />'><a
				href='boardread?bf_idx=${read.bf_idx}&bf_cate_idx=${read.bf_cate_idx}&rpage=${idx}'>${idx}</a>
			</li>
		</c:forEach>
		<c:if test="${next}">
