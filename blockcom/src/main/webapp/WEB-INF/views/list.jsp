...
<body>
	...
	<span class="select" name="searchCondition"
		style="width: 110px; height: 40px;"> <input name="search_t"
		type="hidden"> <select id="searchCondition"
	...
	<span class="input placeholder ml20"
		style="width: 237px; height: 40px;"> <input type="text"
		name="searchValue" id="searchValue" class="i_text"
		...
		<tbody>
			<c:forEach var="row" items="${recList}">
				<tr>
					...
					<c:if test="${use_sec == 'Y' }">
						<td><a href="#"> (비밀글)${row.bf_title} [${row.replyCnt}] </a>
						</td>
					</c:if>
					<c:if test="${use_sec == 'N' }">
						<td><a href="#"> ${row.bf_title} [${row.replyCnt}] </a></td>
					</c:if>
					...
					<c:if test="${date != null}">
						<td><fmt:formatDate value="${row.mod_date}"
								pattern="yyyy-MM-dd" /></td>
					</c:if>
					<td>${row.view_cnt}</td>
	...
	<ul class="pagingUl">
		...
		<c:forEach begin="${startPage}" end="${endPage}" var="idx">
			<li class='<c:out value="${idx == page? 'current':''}" />'><a
				href='boardlist?bf_cate_idx=${bf_cate_idx}&page=${idx}&perPageNumber=${perPageNumber}&blockSize=${blockSize}'
				class="active">${idx}</a>
			</li>
		</c:forEach>
		<c:if test="${next}">
			<li><a
				href='boardlist?bf_cate_idx=${bf_cate_idx}&page=${page + 1}&perPageNumber=${perPageNumber}&blockSize=${blockSize}'>&raquo;</a></li>
		</c:if>
	</ul>
</body>
</html>
